...
from joselib.exceptions import ExpiredSignatureError, JWSError, JWTClaimsError, JWTError
from joselib.utils import calculate_at_hash, timedelta_total_seconds
...
def decode(
    ...
    try:
        ...
        msg = f"Invalid payload string: {err}"
        raise JWTError(msg) from err

    if not isinstance(claims, Mapping):
        msg = "Invalid payload string: must be a json object"
        raise JWTError(msg)
...
def get_unverified_header(token):
    ...
    try:
        ...
        msg = "Error decoding token headers."
        raise JWTError(msg) from err
...
def get_unverified_claims(token):
    ...
    try:
        ...
        msg = "Error decoding token claims."
        raise JWTError(msg) from err

    try:
        claims = json.loads(claims.decode("utf-8"))
    except ValueError as err:
        msg = f"Invalid claims string: {err}"
        raise JWTError(msg) from err

    if not isinstance(claims, Mapping):
        msg = "Invalid claims string: must be a json object"
        raise JWTError(msg)
...
def _validate_iat(claims) -> None:
    ...
    try:
        ...
        msg = "Issued At claim (iat) must be an integer."
        raise JWTClaimsError(msg) from err


def _validate_nbf(claims, leeway=0) -> None:
    ...
    try:
        ...
        msg = "Not Before claim (nbf) must be an integer."
        raise JWTClaimsError(msg) from err
...
def _validate_exp(claims, leeway=0) -> None:
    ...
    try:
        ...
        msg = "Expiration Time claim (exp) must be an integer."
        raise JWTClaimsError(msg) from err
...
def _validate_aud(claims, audience=None) -> None:
    ...
    if not isinstance(audience_claims, list):
        msg = "Invalid claim format in token"
        raise JWTClaimsError(msg)
...
def _validate_iss(claims, issuer=None) -> None:
...
def _validate_sub(claims, subject=None) -> None:
...
def _validate_at_hash(claims, access_token, algorithm) -> None:
    ...
    if claims["at_hash"] != expected_hash:
        msg = "at_hash claim does not match access_token."
        raise JWTClaimsError(msg)
...
def _validate_claims(
    ...
    required_claims = [
        e[len("require_") :] for e in options if e.startswith("require_") and options[e]
    ]
    for require_claim in required_claims:
        if require_claim not in claims:
            msg = f"missing required key `{require_claim}` among claims"
            raise JWTError(msg)
        options[f"verify_{require_claim}"] = True  # override verify when required

    if not isinstance(audience, ((str,), type(None))):
        msg = "audience must be a string or None"
        raise JWTError(msg)
